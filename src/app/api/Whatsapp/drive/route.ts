...
export async function POST(request: Request) {
  try {
    ...
    const keyFilePath = path.join(process.cwd(), 'src', 'app', 'api', 'drive', 'whatsapp-446508-ad0a51269ba9.json');
...
if (mimeType.startsWith('application/') || mimeType.startsWith('audio/')) {
  ...
  const config = {
    ...
    url: `https://graph.facebook.com/v21.0/${process.env.PHONE_ID}/media`,
    headers: {
