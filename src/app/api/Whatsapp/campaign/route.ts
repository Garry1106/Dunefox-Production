...
// =========================================
async function fetchTemplate(templateId: string, request: Request) {
  ...
  for (const contact of contacts) {
    ...
    try {
      const response = await fetch(
        `https://graph.facebook.com/${process.env.WHATSAPP_API_VERSION}/${tenantConfig?.phoneNumberId}/messages`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${process.env.WHATSAPP_ACCESS_TOKEN}`,
...
export async function POST(request: Request) {
  try {
    ...
    const { name, templateId, groupId, scheduledFor } = formSchema.parse(body);
    ...
    if (!scheduledFor) {
      ...
      await sendMessagesToContacts(contacts, templateName, tempLanguage, mediaType, url);
