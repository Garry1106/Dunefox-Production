...
export default function BusinessForm({ form }: BusinessFormProps) {
  ...
  // Handle button click to enable inputs
  const handleCreateWebhook = (e: React.MouseEvent) => {
    e.preventDefault(); // Prevent page refresh
    setIsButtonClicked(true); // Enable the inputs when the button is clicked
    console.log("Webhook Created");
    toast.success("Webhook Created"); // Log a message or trigger an action
  };
    ...
    <Form {...form}>
      <form className="space-y-6">
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          ...
          {/* App ID Field (Disabled until button is clicked) */}
          <FormField
            control={form.control}
            name="appId"
            render={({ field }) => (
